    ...
    for i in tqdm(range(1,nb_pages+1)):
        ...
        for annonce in soup.find_all('div', class_="ergov3-annonce"):
            try:
                ...
                # type de bien
                #print(txt.find('div', class_="ergov3-h3").span.text.split('\n')[1])
                type_bien = txt.find('div', class_="ergov3-h3").span.text.split('\n')[1]

                # nombre de pièce du bien
                nb_pieces = txt.find('div', class_="ergov3-h3").span.text.split('\n')[2]
                nb_pieces = int(nb_pieces)
                
                # surface du bien
                surface = txt.find('div', class_="ergov3-h3").span.text.split(' m²')[0].split(',')[-1]
                surface = int(surface)
                
                # Code postal de l'annonce
                #cp = txt.cite.text.replace("(","").replace(")","").split(' ')[1].replace("\t","").replace("\n","").replace("\r","")
                cp = re.sub(r'[^\d]+', '', txt.find('div', class_="ergov3-h3").cite.text)  
